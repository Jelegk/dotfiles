...
KEY_CUT = "v"
KEY_CANCEL_CUT = "UNMAPPED"
--KEY_CYCLE_ACTION = nil
--KEY_BOOKMARK_ADD = nil
--KEY_CHANNEL_INC = nil
--KEY_CHANNEL_DEC = nil
...
local function cancel_cut()
	text_overlay_off()
	if nil ~= START_TIME then print("CANCELLED CUT") end
	START_TIME = nil
	mp.remove_key_binding('esc_cancel')
end
...
local function put_time()
	...
	if not START_TIME then
		START_TIME = time
		text_overlay_on()
		mp.add_timeout(120, cancel_cut)
		mp.add_forced_key_binding("ESC", "esc_cancel", cancel_cut)
		return
	end
	text_overlay_off()
...
mp.add_forced_key_binding(KEY_CUT, "cut", put_time)
mp.add_forced_key_binding(KEY_CANCEL_CUT, "cancel_cut", cancel_cut)
--mp.add_forced_key_binding(KEY_BOOKMARK_ADD, "bookmark_add", bookmark_add)
--mp.add_forced_key_binding(KEY_CHANNEL_INC, "channel_inc", channel_inc)
--mp.add_forced_key_binding(KEY_CHANNEL_DEC, "channel_dec", channel_dec)
--mp.add_forced_key_binding(KEY_CYCLE_ACTION, "cycle_action", cycle_action)
