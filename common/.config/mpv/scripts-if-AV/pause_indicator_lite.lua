...
local opts = {
    ...
    icon_opacity = 20,               -- Icon opacity (0-100)

    -- pause icon
    rectangles_width = 25,           -- Width of rectangles
    rectangles_height = 60,          -- Height of rectangles
    rectangles_spacing = 15,         -- Spacing between the two rectangles
...
mp.observe_property("pause", "bool", function(_, paused)
    mp.add_timeout(0.1, function()
        if paused then
            ...
            if not opts.indicator_stay then
                mp.add_timeout(opts.indicator_stay_timeout, function()
                    indicator:remove()
                end)
            end
    ...
    if opts.keybind_allow == true then
        mp.set_key_bindings({{opts.keybind_set, function()
            mp.commandv("cycle", "pause")
        end}}, "pause-indicator", "force")
